// X 轴
const xAxisData = ['1KiB', '16KiB', '64KiB', '1MiB', '32MiB'];
...
const c2cGetGbpsData = [
  ...
  [3.02, 14.99, 20.02],
  [3.04, 15.09, 20.02],
  [3.04, 15.00, 19.78] // 32MB
];

const pcclTimeData = [24380, 31500, 348700, 228300, null];
const pcclGbpsData = [0.04, 0.52, 1.88, 4.59, null];
...
const pcclData = pcclTimeData;

// 提取 min/avg/max 列（按 X 轴顺序）
const nbiMin = nbiData.map(d => d[0] * 1000);
const nbiAvg = nbiData.map(d => d[1] * 1000);
const nbiMax = nbiData.map(d => d[2] * 1000);

const blockingMin = blockingData.map(d => d[0] * 1000);
const blockingAvg = blockingData.map(d => d[1] * 1000);
const blockingMax = blockingData.map(d => d[2] * 1000);

const c2cMin = c2cData.map(d => d[0] * 1000);
const c2cAvg = c2cData.map(d => d[1] * 1000);
const c2cMax = c2cData.map(d => d[2] * 1000);
...
option = {
  ...
  legend: [
    ...
    // 第三行：blocking
    {
      data: ['blocking', 'blocking (Avg)', 'blocking (Max)'],
      left: '90%',
      top: '1%',
      textStyle: {
        fontFamily: 'Times New Roman',
